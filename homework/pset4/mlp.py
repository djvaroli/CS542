...
class TwoLayerMLP(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    hidden = h1 # output of the first hidden layer
        
    # [PLEASE IMPLEMENT] 2nd layer activation, N*C
    # hint: involves W2, b2
    z2 = np.dot(h1, W2) + b2 # 2nd layer activation
    scores = z2
    ...
    # output layer
    dscore = P# [PLEASE IMPLEMENT] partial derivative of loss wrt. the logits (dL/dz)
    dscore[range(len(X)), y] -= 1 # difference between observed outputs and one-hot vectors of labels
    ...
    # hidden layer
    dhidden = dscore.dot(W2.T) # d hidden is the derivative wrt to h1
    if self.activation is 'relu':
        f_prime = np.heaviside(z1,0) # derivative of ReLU is the heaviside function
        dh1 = dhidden * f_prime
    elif self.activation is 'sigmoid':
        f_prime = h1*(1 - h1)
        dh1 = dhidden*f_prime
    ...
    # first layer
    dW1 = np.dot(X.T,dh1)/N  # [PLEASE IMPLEMENT]
    db1 = np.mean(dh1,axis = 0)  # [PLEASE IMPLEMENT]
    ###########################################################################
    #                            END OF YOUR CODE
    ###########################################################################
